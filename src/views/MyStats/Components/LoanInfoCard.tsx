...
import type { CollateralType, Loan } from "../../Marketplace/types";
...
import { LoanStatus, LoanTypes } from "../../../config/constants/loans";
...
const LoanInfoCard: React.FC<LoanInfoCardProps> = ({ loanData, isInModal }) => {
  ...
  if (loanData?.canAdjust) {
    loanData?.onAdjust();
    loanData?.setLoanAdjustData(loanData);
  }
  return (
    <LoanInfoWrapper isInModal={isInModal}>
      {isCancel && (
        <CancelLoanDialog
          loanId={loanData.id}
          open={isCancel}
      ...
      <LoanInfoBody>
        ...
        <LoanInfoField>
          <span className="label">AutoSell: </span>'ACTIVE'
          <span className="value">
            {loanData?.autoSell
              ? "On"
